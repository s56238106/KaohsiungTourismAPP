$(document).on("pagecreate", "#page4", function() {
    ...
    function put(){
        ...
        for (var i = 0; i < myObj.length; i++) {
            tabletext += '<a href="#page4_1" id="'+i+'" class="ui-btn ui-corner-all ui-shadow">' + myObj[i].Name + '</a>';
        }
    ...
    function viewaction(){
        for (var i = 0; i < myObj.length; i++) {
            let str='<h3>'+myObj[i].Name+'</h3>'+myObj[i].Description
            let imgsrc=myObj[i].Picture1
            let ii='#'+i;
            $(ii).on("vclick",function(event){
                document.getElementById("showviewdes").innerHTML='<br/><text>'+str+'</text>';
                document.getElementById("viewpic").innerHTML='<img src="'+imgsrc+'" height="50%" width="50%" />';
            });
        }
...
/*    
    ...
    xmlhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            for (var i = 0; i < myObj.length; i++) {
                tabletext += '<tr><td>' + myObj[i].SiteName + '</td><td>' + myObj[i].Temperature + '</td><td>' + myObj[i].Weather + '</td></tr>';
            }
            tabletext += '</table>';
            document.getElementById("demo").innerHTML = tabletext;
        }
    };
    var url = "http://opendata.epa.gov.tw/ws/Data/ATM00698/?$skip=0&$top=1000&format=json";
    xmlhttp.open("GET", url, true);
    xmlhttp.send();
   */ 
});
...
$(document).on("pageshow", "#page7", function() {
    ...
    $("#showviewbutton").on("vclick", function(event) {
        document.getElementById("showviewdiv").innerHTML='<img id="p1_img" src="img/cloud_load1.gif" width="100%" />';
...
$(document).on("pageshow", "#page8", function() {
    ...
    $("#p7_showviewbutton").on("vclick", function(event) {
        document.getElementById("showviewplace").innerHTML='<img id="p1_img" src="img/cloud_load1.gif" width="100%" />';
